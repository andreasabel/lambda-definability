...
abstract
  cons-≡⁺ : ∀ {n i i′ is is′}
    → i ≡ i′
    → (∀ m (m<n : m < n) k k<m → is m m<n k k<m ≡ is′ m m<n k k<m)
    → ∀ m m<n₀ m<n₁
    → cons i is m m<n₀ ≡ cons i′ is′ m m<n₁
  cons-≡⁺ i≡i′ is≡is′ zero m<n₀ m<n₁ = i≡i′
  cons-≡⁺ {is′ = is′} i≡i′ is≡is′ (suc m) m<n₀ m<n₁
    = trans (is≡is′ (nat m) (MS.Sn≤m→n<m m<n₀) m MS.≤-refl)
        (cong (λ p → is′ _ p _ _) (MS.<-prop _ _))


  head-≡⁺ : ∀ {n is is′}
    → (∀ m (m≤n : nat m ≤ n) → is m m≤n ≡ is′ m m≤n)
    → head is ≡ head is′
  head-≡⁺ is≡is′ = is≡is′ 0 MS.0≤n
  ...
  tail-≡⁺ {is′ = is′} is≡is′ m m<n₀ m<n₁ k k≤m₀ k≤m₁
    = trans (is≡is′ (suc k) _) (cong (is′ (suc k)) (≤-prop _ _))
