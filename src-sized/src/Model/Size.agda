...
open import Source.Size.Substitution.Universe using (⟨_⟩)
...
mutual
  ⟦_⟧σ : S.Sub Δ Ω → Functor ⟦ Δ ⟧Δ ⟦ Ω ⟧Δ
  ⟦ Id ⟧σ = Cat.id
  ⟦ σ >> τ ⟧σ = ⟦ τ ⟧σ Cat.∘ ⟦ σ ⟧σ
  ⟦ Wk {n = n} ⟧σ = π₁ n
  ⟦ Fill m m<n ⟧σ = record
    { fobj = λ δ → δ , ⟦ m ⟧n .fobj δ , ⟦<⟧ m<n
    ; fmap = λ δ≤δ′ → δ≤δ′ , ⟦ m ⟧n .fmap δ≤δ′
    }
  ⟦ Keep {n = n} σ refl ⟧σ = record
    { fobj = λ where
        (δ , m , m<n)
          → ⟦ σ ⟧σ .fobj δ , m
...
⟦_⟧σRG : ∀ {Δ Ω} → S.Sub Δ Ω → ⟦ Δ ⟧ΔRG RG.⇒ ⟦ Ω ⟧ΔRG
⟦ σ ⟧σRG = record
  { fobj = ⟦ σ ⟧σ .fobj
  }
