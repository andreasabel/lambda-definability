...
appₛ : ∀ {Δ n} {Γ : ⟦Type⟧ ⟦ Δ ⟧Δ} {T : ⟦Type⟧ ⟦ Δ ∙ n ⟧Δ}
  → ∀ m (m<n : m SS.< n)
  → Γ ⇒ ⟦∀⟧ n T
  → Γ ⇒ subT ⟦ SS.Fill m m<n ⟧σ T
appₛ {T = T} m m<n f = record
  { fobj = λ {δ} x → f .fobj x .arr (⟦ m ⟧n .fobj δ) (MS.⟦<⟧ m<n)
  ; feq = λ δ≈δ′ {x y} x≈y → f .feq _ x≈y _ _ _ _
  }


appₛ∘absₛ : ∀ {Δ n Γ T} m m<n t
  → appₛ {Δ} {n} {Γ} {T} m m<n (absₛ t)
  ≈ subt ⟦ SS.Fill m m<n ⟧σ t
appₛ∘absₛ {Δ} {n} {Γ} {T} m m<n t .≈⁻ γ x = refl


subT-⟦∀⟧ : ∀ {Δ Ω n} (σ : SS.Sub Δ Ω) (T : ⟦Type⟧ ⟦ Ω ∙ n ⟧Δ)
  → ⟦∀⟧ (n [ σ ]n)
      (subT (⟦_⟧σ {Ω = Ω ∙ n} (SS.Keep′ σ)) T)
  ≈⟦Type⟧ subT ⟦ σ ⟧σ (⟦∀⟧ n T)
subT-⟦∀⟧ {Δ} {Ω} {n} σ T = record
  { forth = record
    { fobj = λ {γ} f → record
      { arr = λ m m<n
          → transportObj T
